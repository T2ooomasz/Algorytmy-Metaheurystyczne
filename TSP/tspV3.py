...
def explicit_instance(filename):
    instance = tsplib95.load('/Users/piotrzapala/PycharmProjects/pythonMeta/venv/lib/python3.8/site-packages'
                         '/tsplib95/archives/problems/tsp/' +filename)
    ...
    for i in range(0, length):
        ...
        matrix[edges[i][0]][edges[i][1]] = weight[i]
...
def read_file(filename):
    ...
    if line.find("EUC_2D") != -1:
        ...
        number_of_cities, distance_matrix = explicit_instance(filename)
    else:
        raise Exception
        ...
        try:
            if opt[0][1] == 'load':
                if opt[1][1] == 'symetric':
                    ...
                    n, random_matrix = euclidean_random_instance(number_of_cities, max_x, max_y)
                    while True:
                        sys.stdout.write("1. Calculate k-random\n")
                        sys.stdout.write("2. Calculate nearest neighbor\n")
                        sys.stdout.write("3. Calculate weight of tour\n")
                        ...
                        if option == 1:
                            ...
                            i = int(input().strip())
                            tour = nearest_neighbor(n, i, random_matrix)
                            best_tour = nearest_swap_neighbor(tour, random_matrix)
                            print(best_tour)
                            best_weight = get_weight(best_tour, random_matrix)
                            print(best_weight)      
                        else:
                            sys.exit(2)          
                    ...
                    sys.stdout.write("Enter max distance:")
                    max_distance = int(input().strip())
                    n, random_matrix = asymetric_random_instance(number_of_cities, min_distance, max_distance)
                    while True:
                        sys.stdout.write("1. Calculate k-random\n")
                        sys.stdout.write("2. Calculate nearest neighbor\n")
                        sys.stdout.write("3. Calculate weight of tour\n")
